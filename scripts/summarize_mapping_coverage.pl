...
#  example: perl summarize_mapping_coverage.pl -t sample_tag -r refseq.fa(.gz) -s samstat.out(.gz) -d depth.out(.gz) -c 5 -o sample.coverage_summary.txt
##############################################################
...
my $depth_fh = read_file($depth);
my %depth_stat = parse_depth_file_Nfiltered($depth_fh, $min_depth_cutoff, \%refseq);
close $depth_fh;
